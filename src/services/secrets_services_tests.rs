...
/// Verifies that `validate_secrets` accepts a secrets payload that conforms to a given JSON schema.
///
/// The test builds an object schema requiring `api_key` and `db_password` (both strings)
/// and asserts the validation result is marked valid and contains a `details` field.
///
/// # Examples
///
/// ```
...
/// });
///
/// let output = validate_secrets(&schema, &secrets).expect("validation output should be generated");
