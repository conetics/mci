...
async fn put_stream_uploads_object() -> Result<()> {
    ...
    let key = "hello.txt";
    let body = byte_stream::ByteStream::from_static(b"hello from put_stream");

    utils::s3::put_stream(&client, &bucket, key, body, None).await?;
...
async fn put_stream_validates_digest() -> Result<()> {
    ...
    utils::s3::put_stream(
        ...
        &bucket,
        key,
        byte_stream::ByteStream::from_static(&content[..]),
        Some(&expected),
    )
    .await?;
...
async fn put_stream_rejects_bad_digest() -> Result<()> {
    ...
    let body = byte_stream::ByteStream::from_static(b"oops");

    let result = utils::s3::put_stream(
        &client,
        &bucket,
        key,
        body,
...
async fn put_stream_errors_on_invalid_digest_format() -> Result<()> {
    ...
    let body = byte_stream::ByteStream::from_static(b"content");

    let result = utils::s3::put_stream(&client, &bucket, key, body, Some("badformat")).await;
...
async fn put_stream_errors_on_unsupported_algorithm() -> Result<()> {
    ...
    let result = utils::s3::put_stream(&client, &bucket, key, body, Some("md5:abcd")).await;
...
async fn put_stream_handles_empty_body() -> Result<()> {
    ...
    let key = "empty.txt";
    let body = byte_stream::ByteStream::from_static(b"");
